...
void initIcon(Icon *icon, int iconId, double radius, double angle,
...
void renderScene() {
  ...
  } while (i == j || deckGlobal.cards[j].icons == card1.icons ||
           deckGlobal.cards[i].icons == card2.icons);
  ...
  for (angle = 0.; angle < 360.;
       angle += 360. / (float)((deckGlobal.nbIcons) - 1)) {
    rotation = sin(angle) * angle + 120.;

    drawIcon(currentCard, card1.icons[currentIcon].iconId, radius, angle,
             rotation, scale, &cx, &cy);
    currentIcon++;
    // (cx, cy) est le centre de l'icône placé à l'écran (en pixels)
  }
  rotation = 120.;
  drawIcon(currentCard, card1.icons[currentIcon].iconId, 0., angle, rotation,
           scale, &cx, &cy);
  ...
  for (angle = 0.; angle < 360.;
    ...
    drawIcon(currentCard, card2.icons[currentIcon].iconId, radius, angle,
             rotation, scale, NULL, NULL);
    currentIcon++;
  }
  rotation = 70.;
  drawIcon(currentCard, card2.icons[currentIcon].iconId, 0., angle, rotation,
           scale, NULL, NULL);
